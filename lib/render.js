...
export function render(view, opts = {}) {
  ...
  const modelLine =
    c.border(f.v) +
    fitLR(" MODEL   " + view.model, "", INNER) +
    c.border(f.v);
  const effortLine =
    c.border(f.v) +
    fitLR(" EFFORT  " + view.effort, "GOLD  " + formatCostUsd(view.cost) + " ", INNER) +
    c.border(f.v);
  ...
  const box = [top, modelLine, effortLine, sep, ...meterLines, bottom];
  ...
  // state stays exactly the 8-line box.
  const trailer = [];
