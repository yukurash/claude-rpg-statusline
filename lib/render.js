...
const FRAME = {
  ...
  // Understated single-line frame for the plain theme.
  plain: {
    tl: "┌", tr: "┐", bl: "└", br: "┘",
    h: "─", v: "│",
    sepL: "├", sepR: "┤", down: "┬", up: "┴", hl: "─", iv: "│",
...
// Pure renderer.
// opts = { mode: "truecolor"|"256"|"none", ascii: bool, theme: "rpg"|"plain" }.
// The plain theme keeps the same layout and data but drops the game fiction:
// no HP/MP/BAG prefixes, no level, no ailment badges, COST instead of GOLD,
// neutral column headers and a single-line frame.
export function render(view, opts = {}) {
  const plain = opts.theme === "plain";
  const f = opts.ascii ? FRAME.ascii : plain ? FRAME.plain : FRAME.unicode;
  ...
  const modelLine =
    ...
    fitLR(
      ...
      !plain && view.lv != null ? c.accent("Lv." + view.lv) + " " : "",
      INNER
    ) +
    c.border(f.v);

  // Current git branch: the QUEST you are on (plain theme calls it BRANCH).
  // Only rendered when known, so the box stays 9 lines outside a repo.
  const branchLine = view.branch
    ? c.border(f.v) +
      fitLR(
        " " + c.dim(plain ? "BRANCH" : "QUEST ") + "  " + view.branch,
        "",
        INNER
      ) +
      c.border(f.v)
  ...
  // Status-ailment badges: colored like their DQ counterparts. Plain theme
  // relies on the gauge colors and the warning line instead.
  const BADGE_COLOR = { PSN: c.green, PAR: c.yellow, CRS: c.red };
  const badgeTxt = plain
    ? ""
    : (view.badges || [])
  ...
  const effortLine =
    ...
    fitLR(
      ...
      c.accent((plain ? "COST  " : "GOLD  ") + formatCostUsd(view.cost)) + " ",
      INNER
    ) +
    c.border(f.v);

  // Column header. The gauge and LEFT number show what *remains* (the bar
  // drains as you spend); the last column is time until the limit refills.
  // Plain theme says it without the game vocabulary.
  const colHead =
    c.border(f.v) +
    c.dim(" " + padRight("RESOURCE", COL.name - 1)) + c.border(f.iv) +
    c.dim(center(plain ? "REMAINING" : "LIFE", COL.gauge)) + c.border(f.iv) +
    c.dim(center("LEFT", COL.used)) + c.border(f.iv) +
    c.dim(center(plain ? "RESET" : "RECOVER", COL.reset)) +
    c.border(f.v);
  ...
  const meterLines = view.rows.map((r, i) => {
    ...
    const label = !plain && r.stat ? padRight(r.stat, 4) + r.name : r.name;
  ...
  const trailer = [];
  if (opts.event) trailer.push(c.accent(plain ? "  • " + opts.event : "  ★ " + opts.event));
  const dmsg = dangerMessage(view, opts.lang || "en", opts.theme);
