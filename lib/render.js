...
export function parseInput(raw, ctx = {}) {
  ...
  return {
    ...
    cost: cost.total_cost_usd != null ? cost.total_cost_usd : null,
    rows,
  };
}
...
export function render(view, opts = {}) {
  ...
  const modelLine =
    c.border(f.v) +
    fitLR(" " + c.dim("MODEL") + "   " + view.model, "", INNER) +
    c.border(f.v);
  const effortLine =
    c.border(f.v) +
    fitLR(
      " " + c.dim("EFFORT") + "  " + view.effort,
      c.accent("GOLD  " + formatCostUsd(view.cost)) + " ",
      INNER
    ) +
    c.border(f.v);

  // Column header so each column's meaning is clear (USAGE bar, USED number,
  // RECOVER = time until the limit resets/refills).
  const colHead =
    c.border(f.v) +
    c.dim(" " + padRight("RESOURCE", COL.name - 1)) + c.border(f.iv) +
    c.dim(center("USAGE", COL.gauge)) + c.border(f.iv) +
    c.dim(center("USED", COL.used)) + c.border(f.iv) +
  ...
  const meterLines = view.rows.map((r, i) => {
    ...
    const rawBar = gauge(r.pct, BAR, opts.glyphs);
    const cell2 = " " + c.byPct(rawBar, r.pct) + " ";
    const usedTxt = r.pct == null ? "--/--" : `${r.pct}/100`;
    const cell3 = c.byPct(center(usedTxt, COL.used), r.pct);
