...
// Plain-theme wording: same triggers, no game fiction. English only.
const PLAIN = {
  inventoryFull: "context nearly full — run /compact",
  depleted: (name) => `${name} limit reached — waiting for reset`,
  danger: (name, pct) => `${name} running low — ${100 - pct}/100 left`,
  died: () => null, // the persistent warning line already covers it
  revived: (name) => `${name} limit restored`,
};

// Missing keys in a translation fall back to English.
function pick(lang, theme) {
...
export function dangerMessage(view, lang = "en", theme = "rpg") {
  const t = pick(lang, theme);
...
export function depletionTransition(state, rows, lang = "en", theme = "rpg") {
  const t = pick(lang, theme);
