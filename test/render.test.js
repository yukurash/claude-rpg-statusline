...
test("reproduces the approved mock content (mode none)", () => {
  ...
  // Column header row.
  assert.match(lines[4], /RESOURCE.*USAGE.*USED.*RECOVER/);
  // Gauges reflect usage percentages (default pips style).
  assert.ok(lines[5].includes("▰▰▰▰▰▰▰▰▰▰▰▱▱▱▱▱")); // 71%
  assert.ok(lines[6].includes("▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▱")); // 91%
  assert.ok(lines[7].includes("▰▰▰▰▰▰▰▰▱▱▱▱▱▱▱▱")); // 52%
  // Used cells.
  assert.ok(lines[5].includes("71/100"));
  assert.ok(lines[6].includes("91/100"));
  assert.ok(lines[7].includes("52/100"));
});

test("cursor points at the highest-usage available row", () => {
  const lines = render(normal, { mode: "none" }).split("\n");
  assert.ok(lines[6].startsWith("║▶5-Hour"), "cursor on 5-Hour (91%)");
  assert.ok(lines[5].startsWith("║ Weekly"), "no cursor on Weekly");
  assert.ok(lines[7].startsWith("║ Context"), "no cursor on Context");
});
